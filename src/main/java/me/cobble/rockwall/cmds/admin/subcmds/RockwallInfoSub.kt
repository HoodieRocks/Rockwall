...
class RockwallInfoSub(private val plugin: Rockwall) : RockwallBaseCommand {
    ...
    override fun run(p: Player, args: Array<String>) {
        ...
        p.sendMessage(TextUtils.color("&7Global Chat enabled: ${if (TextUtils.placeholderAPIPresent) "&d" else "&c"}${ChatUtils.isGlobalChatEnabled()}"))
        p.sendMessage(TextUtils.color("&7Parties enabled: ${if (TextUtils.placeholderAPIPresent) "&d" else "&c"}${PartyUtils.arePartiesEnabled()}"))
